...
//	videoConfig->FocalLength = 531.15f;
//	return S_OK;
//}
...
//	depthConfig->FocalLength = 285.63f;
//	return S_OK;
//}
HRESULT KinectSensor::GetVideoConfiguration(FT_CAMERA_CONFIG* videoConfig)
{
...
HRESULT KinectSensor::Init(NUI_IMAGE_TYPE depthType, NUI_IMAGE_RESOLUTION depthRes, BOOL bNearMode, BOOL bFallbackToDefault, NUI_IMAGE_TYPE colorType, NUI_IMAGE_RESOLUTION colorRes, BOOL bSeatedSkeletonMode)
{
	...
	//dxnote: no need to do the full initilization. just need to open any stream in the kinect 1 for face tracker library to work.
	hr = NuiInitialize(NUI_INITIALIZE_FLAG_USES_AUDIO);
	if (FAILED(hr))
	{
		return hr;
	}
